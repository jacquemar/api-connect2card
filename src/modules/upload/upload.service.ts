...
export class UploadService {
  async uploadBanniere(file: Express.Multer.File): Promise<{ url: string }> {
    try {
      ...
      throw new Error("Une erreur s'est produite lors du téléchargement de la bannière.");
    }
  }
  ...
  async uploadPhotoProfil(file: Express.Multer.File): Promise<{ url: string }> {
    try {
      ...
      throw new Error("Une erreur s'est produite lors du téléchargement de la photo de profil.");
    }
  }
} 